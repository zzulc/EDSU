...
class TrainSubgraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sup_neg_tail_ent = [np.random.choice(np.delete(np.arange(nentity), hr2t[(h, r)]),
                                             self.args.metatrain_num_neg) for h, r, t in sup_tri]

        sup_neg_head_ent = [np.random.choice(np.delete(np.arange(nentity), rt2h[(r, t)]),
                                             self.args.metatrain_num_neg) for h, r, t in sup_tri]
        ...
        sup_tri = torch.from_numpy(np.array(sup_tri))
        sup_neg_tail_ent = torch.from_numpy(np.array(sup_neg_tail_ent))
        sup_neg_head_ent = torch.from_numpy(np.array(sup_neg_head_ent))
        que_tri = torch.from_numpy(np.array(que_tri))
        que_neg_tail_ent = torch.from_numpy(np.array(que_neg_tail_ent))
        que_neg_head_ent = torch.from_numpy(np.array(que_neg_head_ent))

        return sup_tri, sup_neg_tail_ent, sup_neg_head_ent, \
               que_tri, que_neg_tail_ent, que_neg_head_ent
