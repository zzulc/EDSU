...
class Trainer(object):
    def __init__(self, args):
        ...
        self.dsg = DSG(args).to(args.gpu)
        self.kge_model = KGEModel(args).to(args.gpu)

        self.sup_weight = nn.Parameter(torch.ones(1)).to(args.gpu)
        self.que_weight = nn.Parameter(torch.ones(1)).to(args.gpu)
        self.rel_weight = nn.Parameter(torch.ones(1)).to(args.gpu)
        self.weight = nn.Parameter(torch.ones(1)).to(args.gpu)

    def save_checkpoint(self, step):
        state = {'ent_init': self.ent_init.state_dict(),
                 'rgcn': self.rgcn.state_dict(),
                 'dsg': self.dsg.state_dict(),
    ...
    def get_loss(self, tri, neg_tail_ent, neg_head_ent, ent_emb, ent_emb_dsg):
        ...
        pos_score_dsg = self.kge_model(tri, ent_emb_dsg)
        loss_reg = (pos_score - pos_score_dsg)**2
    ...
    def get_ent_emb(self, sup_g_bidir):
        ...
        ent_emb, ent_emb_dsg = self.dsg(sup_g_bidir)
        return ent_emb, ent_emb_dsg
    ...
    def evaluate_indtest_test_triples(self, num_cand='all'):
        ...
        ent_emb, ent_emb_dsg = self.get_ent_emb(self.indtest_train_g)
