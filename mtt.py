...
class MetaTrainer(Trainer):
    def __init__(self, args):
        ...
        self.optimizer = optim.Adam(list(self.ent_init.parameters()) + list(self.rgcn.parameters())
                                    + list(self.dsg.parameters()) + list(self.kge_model.parameters()),
                                    lr=self.args.metatrain_lr)
    ...
    def load_pretrain(self):
        ...
        self.dsg.load_state_dict(state['dsg'])
        ...
        for e in range(self.args.metatrain_num_epoch):
            for batch in self.train_subgraph_dataloader:
                ...
                for batch_i, data in enumerate(batch):
                    self.get_ent_emb(sup_g_list[batch_i])
                    sup_tri, sup_neg_tail_ent, sup_neg_head_ent = [d.to(self.args.gpu) for d in data[:3]]
                    que_tri, que_neg_tail_ent, que_neg_head_ent = [d.to(self.args.gpu) for d in data[3:]]
                    ent_emb = sup_g_list[batch_i].ndata['h']
                    ent_emb_dsg = sup_g_list[batch_i].ndata['g']
                    # kge loss
                    loss_task_sup, loss_reg = self.get_loss(sup_tri, sup_neg_tail_ent, sup_neg_head_ent, ent_emb_dsg, ent_emb)
                    loss_task_que, loss_reg_que = self.get_loss(que_tri, que_neg_tail_ent, que_neg_head_ent, ent_emb, ent_emb_dsg)
                    
                    batch_loss = batch_loss + loss_task_sup + loss_task_que + 0.5*loss_reg
                ...
                step += 1
                self.logger.info('step: {} | loss: {:.4f}'.format(step, loss.item()))
                self.write_training_loss(loss.item(), step)
